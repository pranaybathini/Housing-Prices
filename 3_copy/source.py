...
with open('actual_train.csv') as csvfile:
	...
	#params = {'min_child_weight': [1],'n_estimators':list(range(300, 600,10)),'learning_rate':[0.1],'gamma':[i/10.0 for i in range(0,5)]}
	import xgboost as xgb
	est=list(range(4000,8500,500))
	lr=[0.08,0.1]
	md=[2,3]
	#gamma=[0.0,1e-5,1e-4,1e-3]
	for i in md:
		for j in lr:
			for k in est:
				xclass = XGBRegressor()
				#xclass = GridSearchCV(xcl,params)
				xclass.fit(X_train ,y_train)
				prediction5 = xclass.predict(X_test)
				print("r2 val for %d depth %f rate %d estimators %f"%(i,j,k,r2_score(y_test,prediction5)))
