...
with open('actual_train.csv') as csvfile:
	data = pd.read_csv(csvfile,delimiter=',')  #creating a pandas dataframe
	
	
	
	
	data['Builder Name'].fillna(np.nan,inplace=True)		#filling mising values with np.nan
	lbl = preprocessing.LabelEncoder()					#alternatively fill with "Nan" and use label encoding to handle
	lbl.fit(list(data['Builder Name'].values))
	data['Builder Name'] = lbl.transform(list(data['Builder Name'].values))

	
	for x in data['Date Built']:			#convert time string into valid format
		df = datetime.datetime.strptime(x, "%m/%d/%Y %I:%M %p").strftime("%m/%d/%Y %H:%M")	#I : 24 hr , H:12 hr format
		data['Date Built'].replace(x,df)		#replace old string with new valid one 
	data['Date Built'] = pd.to_datetime(data['Date Built']) #convert it into datetime object
	data['Date Built'] = (data['Date Built'] - data['Date Built'].min())  / np.timedelta64(1,'D')	
								#it will return a float value,np.timedelta  complements datetime
	
	for xyy in data['Date Priced']:		#convert time string into valid format
		df = datetime.datetime.strptime(xyy, "%m/%d/%Y %I:%M %p").strftime("%m/%d/%Y %H:%M")
		data['Date Priced'].replace(xyy,df)	
	data['Date Priced'] = pd.to_datetime(data['Date Priced'])	 #convert it into datetime object
	data['Date Priced'] = (data['Date Priced'] - data['Date Priced'].min())  / np.timedelta64(1,'D')
	

	data['garden'].fillna(np.nan,inplace=True)#if xgbboost it will handle missing values itself ,alternatively we can use rolling window for 0 or 1 data
	...
	#if xgbboost it will handle missing values itself ,alternatively we can use rolling window for 0 or 1 data
	...
	data['land'].fillna(np.nan,inplace=True)	#use label Encoding to handle strings
	lbl = preprocessing.LabelEncoder()
	lbl.fit(list(data['land'].values))
	data['land'] = lbl.transform(list(data['land'].values))
	
	data['Location'].fillna(np.nan,inplace=True)	#use label Encoding to handle strings
	lbl = preprocessing.LabelEncoder()
	...
	y = data['Golden Grains']	#storing target feature in y
	del data['Golden Grains']
	del data['House ID'] 	
	X = data					#storing feature vector in x
	
	with open('test.csv') as TestData:
		data1 = pd.read_csv(TestData,delimiter=',')
		
		data1['Builder Name'].fillna(np.nan,inplace=True)	#filling mising values with np.nan
		lbl = preprocessing.LabelEncoder()
		lbl.fit(list(data1['Builder Name'].values))		#alternatively fill with "Nan" and use label encoding to handle
		data1['Builder Name'] = lbl.transform(list(data1['Builder Name'].values))

	
		for x in data1['Date Built']:		#convert time string into valid format
		...
		data1['Date Built'] = (data1['Date Built'] - data1['Date Built'].min())  / np.timedelta64(1,'D')
									...
									#convert time string into valid format
		...
		data1['garden'].fillna(np.nan,inplace=True)	#filling mising values with np.nan
		#data1['garden'] = pd.DataFrame(data1['garden'])
		#data1['garden'].fillna(0).rolling(window=2,min_periods=1).max()
		
		#filling mising values with np.nan
		...
		#alternatively fill with "Nan" and use label encoding to handle
		...
		from xgboost.sklearn import XGBRegressor	#using xgb regressor to predict 
		clf=XGBRegressor(learning_rate=0.0799,max_depth=2,n_estimators=9000)
		clf.fit(X,y)
		prediction1=clf.predict(data1)
